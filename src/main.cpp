...
void setServoFromError(float targetFrequency, float measuredFrequency)
{
  const float delta = measuredFrequency - targetFrequency;
  int servoOffset = static_cast<int>(delta * SERVO_GAIN);
  servoOffset = constrain(servoOffset, -static_cast<int>(SERVO_MAX_OFFSET), static_cast<int>(SERVO_MAX_OFFSET));
  pwmDriver.setPWM(SERVO_CHANNEL, 0, SERVO_CENTER + servoOffset);
}

void centerServo()
{
  pwmDriver.setPWM(SERVO_CHANNEL, 0, SERVO_CENTER);
}
...
void processCurrentString()
{
  ...
  setServoFromError(targetFrequency, static_cast<float>(measuredFrequency));
